...
class Swift_Transport_MailTransport implements Swift_Transport
{
    ...
    public function send(Swift_Mime_Message $message, &$failedRecipients = null)
    {
        ...
        if ("\r\n" != PHP_EOL) {
            ...
            // CUSTOM BUG FIX PATCH - PHP 8 mail() function handles line end characters differently causing raw HTML to
            // ... display in messages.  So we only call this code for PHP versions before v8.0
            /* References:
                  https://github.com/php/php-src/issues/8086
                  https://www.drupal.org/project/webform/issues/3239517
                  https://www.php.net/manual/en/function.mail.php - See note on page: 
                      Note: If messages are not received, try using a LF (\n) only. Some Unix mail transfer agents (most
                      notably » qmail) replace LF by CRLF automatically (which leads to doubling CR if CRLF is used).
                      This should be a last resort, as it does not comply with » RFC 2822.
                  https://bugs.php.net/bug.php?id=47983
            */
            if (version_compare(phpversion(), 8) < 0) { // starting PHP 8 this can cause incorrect line endings in UNIX
                $headers = str_replace("\r\n", PHP_EOL, $headers);
                $subject = str_replace("\r\n", PHP_EOL, $subject);
                $body    = str_replace("\r\n", PHP_EOL, $body);
                $to      = str_replace("\r\n", PHP_EOL, $to);
